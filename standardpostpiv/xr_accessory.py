...
from .statistics import running_relative_standard_deviation
...
class StdPIVDAAccessor:
    ...
    def compute_running_relative_standard_deviation(self, dim, ddof=0) -> xr.DataArray:
        ...
        for d in self._obj.dims:
            ...
            dim_axis += 1
        dims = self._obj.dims
        return xr.DataArray(running_relative_standard_deviation(self._obj.to_numpy(), axis=dim_axis, ddof=ddof),
                            dims=dims,
                            coords={d: self._obj.coords[d] for d in dims},
                            attrs={'standard_name': f'running_relative_standard_deviation_of_{self._obj.name}',
                                   'units': ''})
